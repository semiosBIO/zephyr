...
static int flash_ambiq_erase(const struct device *dev, off_t offset, size_t len)
{
	...
	for (int i = 0; i < len / FLASH_ERASE_BLOCK_SIZE; i++) {
		uint32_t flash_addr = SOC_NV_FLASH_ADDR + offset + i * FLASH_ERASE_BLOCK_SIZE;

		ret = am_hal_flash_page_erase(AM_HAL_FLASH_PROGRAM_KEY,
						AM_HAL_FLASH_ADDR2INST(flash_addr),
						AM_HAL_FLASH_ADDR2PAGE(flash_addr));
		if (ret) {
			break;
		}
	}
